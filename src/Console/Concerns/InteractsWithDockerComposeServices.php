...
namespace Harmony\Sail\Console\Concerns;
...
trait InteractsWithDockerComposeServices
{
    ...
    {
        if (function_exists('\Harmony\Prompts\multiselect')) {
            return \Harmony\Prompts\multiselect(
    ...
    protected function buildDockerCompose(array $services)
    {
        ...
        // Adds the new services as dependencies of the Harmony.test service...
        if (! array_key_exists('Harmony.test', $compose['services'])) {
            $this->warn('Couldn\'t find the Harmony.test service. Make sure you add ['.implode(',', $services).'] to the depends_on config.');
        } else {
            $compose['services']['Harmony.test']['depends_on'] = collect($compose['services']['Harmony.test']['depends_on'] ?? [])
                ->merge($services)
                ->unique()
                ->values()
                ->all();
        ...
        file_put_contents($this->Harmony->basePath('docker-compose.yml'), Yaml::dump($compose, Yaml::DUMP_OBJECT_AS_MAP));
    }
    ...
    {
        $environment = file_get_contents($this->Harmony->basePath('.env'));
        ...
        file_put_contents($this->Harmony->basePath('.env'), $environment);
    }
    ...
    {
        if (! file_exists($path = $this->Harmony->basePath('phpunit.xml'))) {
            $path = $this->Harmony->basePath('phpunit.xml.dist');
        }
        ...
        file_put_contents($this->Harmony->basePath('phpunit.xml'), $phpunit);
    }
    ...
    {
        if (! is_dir($this->Harmony->basePath('.devcontainer'))) {
            mkdir($this->Harmony->basePath('.devcontainer'), 0755, true);
        }

        file_put_contents(
            $this->Harmony->basePath('.devcontainer/devcontainer.json'),
